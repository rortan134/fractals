...
def plot_line(from_coordinates, to_coordinates, thickness, colour, pixels):
    ...
    distance = math.sqrt((to_coordinates[1] - from_coordinates[1])
                         ** 2 + (to_coordinates[0] - from_coordinates[0])**2)
    step = round(distance)
...
def draw_triangle(center, side_length, degrees_rotate, thickness, colour, pixels, shrink_side_by, iteration, max_depth):
    ...
    # Bottom left corner
    bottom_left_corner = [center[0] +
                          triangle_height/2, center[1] - side_length/2]
    # Bottom right corner
    bottom_right_corner = [center[0] +
                           triangle_height/2, center[1] + side_length/2]
    if (degrees_rotate != 0):
        top_corner = rotate_coordinate_around_point(
            top_corner, center, degrees_rotate)
        bottom_left_corner = rotate_coordinate_around_point(
            bottom_left_corner, center, degrees_rotate)
        bottom_right_corner = rotate_coordinate_around_point(
            bottom_right_corner, center, degrees_rotate)

    lines = [[top_corner, bottom_left_corner], [top_corner,
                                                bottom_right_corner], [bottom_left_corner, bottom_right_corner]]
    line_number = 0
    ...
    for line in lines:
        ...
        if (iteration < max_depth and (iteration < 1 or line_number < 3)):
            ...
            if (gradient < 0.0001 and gradient > -0.0001):
                ...
                x_length = math.sqrt(
                    (distance_from_center**2)/(1 + difference_from_center**2))
                ...
                new_center = [center_of_line[0] +
                              y_length, center_of_line[1] + x_length]

            draw_triangle(new_center, new_side_length, new_rotation, thickness,
                          colour, pixels, shrink_side_by, iteration+1, max_depth)
...
# Draw 4 fractals
draw_triangle([3000, 3000], 2000, 0, 8, [255, 200, 0], pixels, 1/2, 0, 9)
draw_triangle([3000, 7000], 1200, 0, 2, [165, 242, 243], pixels, 2/3, 0, 9)
draw_triangle([7000, 3000], 1000, 0, 2, [124, 252, 0], pixels, 2.28/3, 0, 9)
draw_triangle([7000, 7000], 800, 0, 2, [203, 195, 227], pixels, 2.5/3, 0, 9)
